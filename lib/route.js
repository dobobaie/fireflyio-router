...
module.exports = class Route
{
  ...
  retrieveRouteDetails(method, custom_route) {
  ...
  push(method, route, ...middlewares) {
    const existingMiddlewares = this.fireflyio.modules.middleware.retrieveMiddlewares();
    const route_analyse = this._routeAnalysis(route);
    this.routes[method + route_analyse.access_route] = {
      method,
      route,
      route_analyse,
      middlewares: [...existingMiddlewares, ...middlewares]
    };
    return this;
  };

  pushMiddlewareToAllRoutes(middleware) {
    Object.keys(this.routes).forEach(route_key =>
      this.routes[route_key].middlewares.push(middleware)
    );
  }
};
